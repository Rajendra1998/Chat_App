...
public class UsernameServlet extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(session!=null)
		{
			...
			out.println("<input type=\"text\" id=\"messageText\" onkeypress=\"myFunction(event)\" placeholder=\"Type a message\" size=\"50\" />");
			out.println("<input type=\"button\" value=\"Send\"  onclick=\"sendMessage();\" />");
			out.println("<br><br><a href=\"Home1.html\"><i>Click here to exit Chat</i></a>");
			out.println("<script>");
				out.println("function myFunction(event) {");
				out.println(" var x = event.which; ");
				out.println("if(x==13){sendMessage();}");
				out.println("}");
			out.println("var websocket=new WebSocket(\"ws://192.168.43.159:8082/Chat_App/ChatRoomServerEndPoint\");");
